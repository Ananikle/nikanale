...
/**
 * \brief Количество элементов, не оканчивающихся на 0.
 * \param arr - Указатель на старый массив.
 * \param n - Размер старого массива.
 * \return - Размер нового массива.
 */
size_t NewArraySize(const int* arr, const size_t n);

/**
 * \brief Удаление элементов оканчивающихся на 0.
 * \param arr - Указатель на старый массив.
 * \param n - Размер старого массива.
 * \param newn - Размер нового массива.
 * \return - Адрес нового массива.
 */
int* DelLastZeros(const int* arr, const size_t n, const size_t newn);
...
int main()
{
    ...
    size_t n1 = NewArraySize(arr, n);
    int* arr1 = DelLastZeros(arr, n, n1);
    PrintArray(arr1, n1);

    int* arr2 = FormNewArray(arr1, n1);
...
int* DelLastZeros(const int* arr, const size_t n, const size_t newn) {
    cout << "Массив без элементов, оканчивающихся на 0: " << endl;
